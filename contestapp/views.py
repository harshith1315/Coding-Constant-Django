...
def home(request, qno=None):
    ...
    #leaderboard
    score = score  # Assuming you want to store the inverse of the score

    # Update the leaderboard for the logged-in user
    user = request.user
    leaderboard_entry, created = models.Leaderboard.objects.get_or_create(user=user, defaults={'score': 0})
...
def leaderboard(request):
    leaderboard_entries = models.Leaderboard.objects.all().order_by('-score')[:10]  # Get top 10 entries
